		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("officeuuid") === oObject.officeuuid;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].officeuuid;
									} else {
										sCurrentId = aItemsNotInTheList[0].officeuuid;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("officeuuid"),
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "ZCPDB_OFFICE_TP (0)"
						}),
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'ZCPDB_OFFICE_TP (0)'");
						},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all ZCPDB_OFFICE_TP to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "ZCPDB_OFFICE_TP (" + iExpectedLength + ")"
								}),
								success: function () {
